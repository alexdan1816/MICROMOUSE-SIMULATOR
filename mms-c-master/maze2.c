...
void printQueue(LIFOqueue *queue)
{
    char buffer[50];
    log("Current Queue:");
    ...
    {
        snprintf(buffer, sizeof(buffer), "Location %d: (x: %d, y: %d)", i, queue->Q[i]->x, queue->Q[i]->y);
        log(buffer);
    }
}
...
struct location *popQ(struct LIFOqueue **_queue) // take out top cell
{
    if ((*_queue)->top == -1)
    {
        return NULL;
    }
    struct location *takeout = (*_queue)->Q[(*_queue)->top];
    (*_queue)->Q[(*_queue)->top] = NULL;
    (*_queue)->top--;

    return takeout;
}
struct LIFOqueue *addQ(struct LIFOqueue *_queue, short x, short y) // add cell
{
    _queue->top++;
    _queue->Q[_queue->top]->x = x;
    _queue->Q[_queue->top]->y = y;
    return _queue;
}
...
struct maze *reflood(struct LIFOqueue **_queue, struct maze **_maze)
{
    ...
    while ((*_queue)->top != -1)
    {
        ...
        struct location *popped = popQ(_queue);
        ...
        if ((*_maze)->map[xlocate][ylocate]->down != NULL) // if this cell available
        {
            ...
            else // if this cell's value higher or equal to popped cell, take minimum value is this cell's value
            {
                ...
                else if (min > (*_maze)->map[xlocate][ylocate]->down->value)
                {
                    min = (*_maze)->map[xlocate][ylocate]->down->value;
                }
            }
        ...
        {
            if ((*_maze)->map[xlocate][ylocate]->value == (*_maze)->map[xlocate][ylocate]->right->value + 1)
            {
                flag = true;
            }
            else
        ...
        // continue checking lower available neighbour after changing value
        if ((*_maze)->map[xlocate][ylocate]->up != NULL && (*_maze)->map[xlocate][ylocate]->value > (*_maze)->map[xlocate][ylocate]->up->value)
        {
            (*_queue) = addQ(*_queue, (*_maze)->map[xlocate][ylocate]->up->x, (*_maze)->map[xlocate][ylocate]->up->y);
        }

        if ((*_maze)->map[xlocate][ylocate]->down != NULL && (*_maze)->map[xlocate][ylocate]->value > (*_maze)->map[xlocate][ylocate]->down->value)
        {
            (*_queue) = addQ(*_queue, (*_maze)->map[xlocate][ylocate]->down->x, (*_maze)->map[xlocate][ylocate]->down->y);
        }

        if ((*_maze)->map[xlocate][ylocate]->left != NULL && (*_maze)->map[xlocate][ylocate]->value > (*_maze)->map[xlocate][ylocate]->left->value)
        {
            (*_queue) = addQ(*_queue, (*_maze)->map[xlocate][ylocate]->left->x, (*_maze)->map[xlocate][ylocate]->left->y);
        }

        if ((*_maze)->map[xlocate][ylocate]->right != NULL && (*_maze)->map[xlocate][ylocate]->value > (*_maze)->map[xlocate][ylocate]->right->value)
        {
            (*_queue) = addQ(*_queue, (*_maze)->map[xlocate][ylocate]->right->x, (*_maze)->map[xlocate][ylocate]->right->y);
        }
    }

    log("done reflooding, queue is empty");
    return *_maze;
}
// Maze explore
